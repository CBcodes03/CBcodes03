...
`EC2` `S3` `Lambda` `EKS` `ECS` `RDS` `CloudWatch` · `GKE` `Pub/Sub` `Cloud CDN` `Load Balancer`
...
`Image Scanning` `RBAC` `Security Groups`
...
| Area | Topics |
|---|---|
| ☸️ Kubernetes | Platform engineering, operators, multi-cluster |
| 🔁 GitOps | ArgoCD workflows, progressive delivery |
| 📊 Observability | Distributed tracing, alerting, SLOs |
| 🏗️ IaC | Terraform modules, Ansible playbooks |
| ☁️ Cloud | EKS/GKE, cloud-native patterns |
| 🔒 Site Reliability | SRE practices, incident response |
